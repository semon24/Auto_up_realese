...
{
    public const string ClientMethodDockerComposeDown = "docker_compose_down";

    readonly ConcurrentDictionary<string, PendingDockerComposeDown> _dockerComposeDownWaiters = new();

    sealed class PendingDockerComposeDown
    {
        public required Guid Id { get; init; }
        public required TaskCompletionSource<(bool Ok, string? Error, object? Payload)> Tcs { get; init; }
    }

    public async Task<(bool Ok, string? Error, object? Payload)> StartDockerComposeDownWithAgentAsync(
        string hostName,
        string tag,
        TimeSpan timeout,
        CancellationToken cancellationToken)
    {
        ...
        var pending = new PendingDockerComposeDown { Id = id, Tcs = tcs };
        if (!_dockerComposeDownWaiters.TryAdd(normalizedHostName, pending))
        ...
        {
            if (_dockerComposeDownWaiters.TryGetValue(normalizedHostName, out var cur) && ReferenceEquals(cur.Tcs, tcs))
                _dockerComposeDownWaiters.TryRemove(normalizedHostName, out _);
        }
    }

    public void HandleDockerComposeDownCompleted(
        string connectionId,
        string id,
        bool ok,
        string? error,
        object? payload)
    {
        ...
        if (!Guid.TryParse(id, out var msgId))
            return;
        if (!_dockerComposeDownWaiters.TryGetValue(normalizedHostName, out var pending) || pending.Id != msgId)
            return;
