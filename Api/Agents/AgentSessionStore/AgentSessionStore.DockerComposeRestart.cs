...
{
    public const string ClientMethodDockerComposeRestart = "docker_compose_restart";

    readonly ConcurrentDictionary<string, PendingDockerComposeRestart> _dockerComposeRestartWaiters = new();

    sealed class PendingDockerComposeRestart
    {
        public required Guid Id { get; init; }
        public required TaskCompletionSource<(bool Ok, string? Error, object? Payload)> Tcs { get; init; }
    }

    public async Task<(bool Ok, string? Error, object? Payload)> StartDockerComposeRestartWithAgentAsync(
        string hostName,
        string tag,
        TimeSpan timeout,
        CancellationToken cancellationToken)
    {
        ...
        var pending = new PendingDockerComposeRestart { Id = id, Tcs = tcs };
        if (!_dockerComposeRestartWaiters.TryAdd(normalizedHostName, pending))
            return (false, "Команда рестарта уже выполняется", null);
        ...
        {
            if (_dockerComposeRestartWaiters.TryGetValue(normalizedHostName, out var cur) && ReferenceEquals(cur.Tcs, tcs))
                _dockerComposeRestartWaiters.TryRemove(normalizedHostName, out _);
        }
    }

    public void HandleDockerComposeRestartCompleted(
        string connectionId,
        string id,
        bool ok,
        string? error,
        object? payload)
    {
        ...
        if (!Guid.TryParse(id, out var msgId))
            return;
        if (!_dockerComposeRestartWaiters.TryGetValue(normalizedHostName, out var pending) || pending.Id != msgId)
            return;
